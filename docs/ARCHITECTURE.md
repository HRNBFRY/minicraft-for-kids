...
- **虹・オーロラ（Phase2残り）**: `js/core/skyfx.js` の `SkyFX` が担当する純粋な描画エフェクト
  （ブロック・セーブデータには一切触れない）。`game.js` がオーバーワールドの時だけ毎フレーム
  `update(dt, {overworld, gen, playerPos, isNight, weatherEnabled, sunPos})` を呼ぶ。
  - **オーロラ**: `gen`（openworldエンジン）があり、かつ `DayNight.isNight()` かつプレイヤー足元の
    バイオームが寒冷（`OB.SNOWY_*`/`GLACIER`/`FROZEN_OCEAN`）のときだけ、プレイヤー上空に3枚の
    半透明カーテン（加算合成・端は頂点色を黒に近づけてフェード）を表示し、sinで波打たせる。
    条件が揃わない、または `gen` が無い（＝既存ワールド）ときは常に非表示。
  - **虹**: `Weather.enabled`（雨）が有効なとき／直後（余韻タイマー）、または低確率の自然発生
    タイマーで、太陽と反対方向の遠景に6色のトーラス片（半円アーチ）を表示する。`group.lookAt()`
    でプレイヤーの方を向かせるだけで、常に正面から見た円弧になる。
  - どちらも表示/非表示を毎フレーム線形補間してフェードさせ、ポップイン/アウトしないようにする。
  - 従来エンジン（`world.js`/`worldgen.js`）やセーブ処理には一切触れていない。
- **都市生成（Phase3）**: `worldgen.js` の `cityCell()` がランドマークより大きなセル（既定512）
  ごとに確率的（既定35%）に都市の位置・タイプ・規模・基準面（周囲の高さ平均）を決定的に返し、
  `world.js` の `stampCity()` がチャンクに重なる範囲だけを列単位で書き込む（landmark と同じ分割描画方式）。
  - **都市タイプ**（アンカーのバイオームで自動選択）: 石造り都市 / 港町 / 山岳都市 / 島国 / 雪国 /
    火山都市 / 森の村 / 巨大樹の村（`_cityPalette()` が壁・道・屋根の材質を切り替える）。
  - **手順**: まず都市半径内を基準面へ整地（外周は元の地形へなだらかに溶かす）→ 道路グリッド＋
    中央広場（噴水）→ 街区ごとに建物（乱数でサイズ・高さ・窓・段々屋根が変わる）／市場／庭を配置。
    石造り・山岳・火山・雪国タイプは城壁（道路の通る位置に門を自動で開ける）で囲み、火山都市は
    城壁の外に溶岩の堀を持つ。巨大樹の村は広場の代わりに中央へ巨木を生やす。
  - 建物内部は空洞のまま（家具等はPhase4で追加予定）。
- **今後（Phase4）**: Phase3の建物内部に家具（机・椅子・本棚・暖炉・ベッド・宝箱等）を生成する。
  従来エンジンには触れず拡張する。
